...
> Minimal Claude Code statusline — context / token / todos / tools / agents (lightweight claude-hud alternative)
...
| **Node.js** | ≥ 18.0.0 | `node --version` | 用来编译 TypeScript |
| **npm** | ≥ 9.0 | `npm --version` | 装 TypeScript |
| **TypeScript** | ≥ 5.4 | `npx tsc --version` | 编译时自动装 |

> 💡 **零运行时依赖** — 编译产物是纯 Node.js,不依赖任何 npm 包。装一次,跑很久。
...
看到这两行?说明一切正常。下一步:[正式安装到 Claude Code](#安装)。
...
✅ 完成!重启 Claude Code,输入框下方就能看到状态栏。

**setup 流程会**:
1. 检测你的操作系统和 Claude 配置目录
2. 询问语言 (中文 / English)
3. 检查现有 statusLine,如有冲突会备份到 `statusLine.bak.<时间戳>.json`
4. 把 `node <path>/dist/index.js` 写入 `~/.claude/settings.json`
5. 提示重启 + 验证步骤

### 方式 2: 手动 git clone

> 适合想自己控制路径、调试时改代码立即生效的人。

**Linux / macOS**:

```bash
# 1. 克隆到 Claude 插件目录
# 路径约定: {vendor}/{name}/{version}/ — 改版本时同步改这里
git clone https://github.com/zander-zyx/claude-mini-hud.git \
...
> 💡 **目录命名规范**:Claude Code 期望 `{vendor}/{name}/{version}/` 三级结构。`local` 是 vendor,`claude-mini-hud` 是 name,`0.1.0` 是 version。**改代码时不要改 version**,否则 Claude Code 认为是新插件,会重新跑一次缓存逻辑。
...
这是 [Claude Code 平台限制](https://github.com/anthropics/claude-code/issues/14799),不是本插件的问题。
...
**不影响 Claude Code 性能**:渲染是异步的,不到 10ms,不抢主进程 CPU。
...
**A**: `statusLine.command` 路径不对,或 `node` 找不到。
...
**解决**: 检查 `~/.claude/settings.json` 里 `transcriptPath` (Claude config 内部,不是 statusLine) 是否可读。
...
**A**: **不行**。两者都用 `statusLine.command`,后装的覆盖先装的。如果想 A/B 测试,在 `~/.claude/settings.json` 切换 `command` 即可。
